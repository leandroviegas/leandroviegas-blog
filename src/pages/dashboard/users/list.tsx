...
export function Head() {
  return <SeoHead title="Listar usuários - Leandro Viegas" />;
}
...
function Index() {
  ...
  const [users, setUsers] = useState<{ status: Status; data: User[] }>({
    status: "",
    data: [],
  });
  ...
  const HandleDeleteUser = (_id = "") => {
    ...
    api
      .get(selectedUser?.active ? `/users/deactive` : `/users/active`, {
        params: { _id: selectedUser?._id ?? _id },
  ...
  return (
    <DashboardLayout>
      ...
      <DeletePopup
        ...
        errors={alerts["user-delete"]}
        btnText="Desativar usuário"
        open={popup === "delete"}
        ...
        text={`Você tem certeza que deseja desativar esse usuário "${selectedUser?.username}".`}
      />
      ...
      {users.status === "success" && users.data.length > 0 && (
        <div className="container">
          <div className="bg-white dark:bg-zinc-900 rounded-lg p-4 mx-4 my-8 shadow-lg">
            ...
            <div className="my-6">
              <button
                onClick={() => setPopup("user-form")}
                className="shadow-md shadow-indigo-500/30 hover:scale-110 bg-indigo-500 hover:bg-indigo-700 transition font-semibold text-white px-3 py-1 rounded"
              >
                Novo usuário
              </button>
            </div>
                  ...
                  >
                    <div className="grow pt-2 flex flex-col gap-2 pb-2 px-4 w-full text-zinc-700 dark:text-zinc-300 rounded-t-lg break-words">
                      ...
                      <p className="font-thin opacity-80 whitespace-normal">
                        {truncate(user?.about, 100)}
                      </p>
                      <a
                        href={`/blog/user/${user?.link}`}
                        target="_blank"
                    ...
                    <div className="p-3 bg-indigo-500 shadow-md shadow-indigo-500/40 rounded-b-xl font-semibold flex items-center gap-4 flex-wrap">
                      ...
                      {user.active == true ? (
                        ...
                        <button
                          ...
                          className="bg-green-500 text-white hover:bg-green-700 px-3 transition rounded py-1"
                        >
                          Ativar
                        </button>
                      )}
