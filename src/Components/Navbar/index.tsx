...
const Index = () => {
    ...
    const { signIn, signUp, signOut, userInfo } = useAuth()
    ...
    const HandleLogin = (evt: React.FormEvent<HTMLFormElement>) => {
        ...
        if (!(loginForm.usernameOrEmail.length >= 8)) alerts.push({
    ...
    useEffect(() => {
        ...
        document.querySelector("body").style.overflowY = (tab !== "" || showNavbar) ? "hidden" : "auto";
    }, [tab, showNavbar])
                                ...
                                {tab === "SignIn" &&
                                    <>
                                        <div className="grid grid-cols-2 border rounded">
                                            <button className={`font-semibold w-full bg-gradient-to-b from-sky-500 to-cyan-400 hover:from-sky-600 hover:to-cyan-500 text-white py-1 rounded`}>Entrar</button>
                                        ...
                                        <form onSubmit={HandleLogin} className="flex flex-col gap-3 mt-4">
                                            ...
                                            <div className="w-full">
                                                <button type="submit" className="from-sky-500 to-cyan-400 hover:from-sky-600 hover:to-cyan-500 rounded text-lg font-semibold w-full bg-gradient-to-b from-sky-500 to-cyan-400 text-white hover:text-zinc-50 px-3 py-1 hover:bg-cyan-600 transition">{loginFormState === "loading" ? <VscLoading className="animate-spin mx-auto text-2xl" /> : "Entrar"}</button>
                                            </div>
                                        </form>
                                    </>
                                }
                                {tab === "SignUp" &&
                                    <>
                                        <div className="grid grid-cols-2 border rounded">
                                            ...
                                            <button className={`font-semibold w-full bg-gradient-to-b from-sky-500 to-cyan-400 hover:from-sky-600 hover:to-cyan-500 text-white transition py-1 rounded`}>Registrar-se</button>
                                        </div>
                                        <div className="flex justify-center items-center py-8 pb-5">
                                            <span className="absolute text-sm bg-white px-2 text-zinc-500">Redes sociais</span>
                                            <hr className="w-full" />
                                        ...
                                        <form onSubmit={HandleRegister} className="flex flex-col gap-3 mt-4">
                                            ...
                                            <hr className="py-1" />
                                            <div className="w-full">
                                                <button type="submit" className="from-sky-500 to-cyan-400 hover:from-sky-600 hover:to-cyan-500 rounded text-lg font-semibold w-full bg-gradient-to-b from-sky-500 to-cyan-400 text-white hover:text-zinc-50 px-3 py-1 hover:bg-cyan-600 transition">{registerFormState === "loading" ? <VscLoading className="animate-spin mx-auto text-2xl" /> : "Registrar-se"}</button>
                                            </div>
                                        </form>
                                    </>
                                }
            ...
            <nav className="bg-gradient-to-b from-sky-600 to-cyan-400 max-w-screen">
                <div className="container mx-auto flex flex-row justify-between lg:items-center py-4 ">
                    <div className="shrink-0 grow flex flex-col md:flex-row justify-start items-center gap-4 px-2">
                        ...
                        {window.location.pathname.startsWith("/blog") && <form className="w-full md:w-auto flex items-center bg-white h-7 rounded border border-zinc-200 text-sm text-zinc-600">
                    ...
                    <div className={`${showNavbar ? "" : "hidden "} lg:block w-screen h-screen lg:w-auto lg:h-auto fixed lg:relative right-0 top-0 z-20 flex justify-end bg-black/70 lg:bg-transparent`}>
                        <div className={`${showNavbar ? "max-w-64 " : "max-w-0 "}w-full navbar lg:max-w-none lg:block duration-300 h-full bg-white lg:bg-transparent overflow-auto lg:max-w-auto text-gray-800 font-semibold`}>
                            <div className="p-4 lg:p-0 justify-start flex flex-col lg:flex-row lg:items-center gap-4">
                                ...
                                ].map(link => <Link className={`order-1 ${window.location.pathname + window.location.hash === link.href ? "text-black lg:text-white hover:lg:text-white" : "text-zinc-700 hover:text-black hover:lg:text-white/90"}`} to={link.href} key={link.value + link.href}>{link.value}</Link>)}
                                        ...
                                        <div>
                                            <Outclick callback={() => setUserDropdown(false)}>
                                                ...
                                                <div className={`${userDropdown ? "max-h-auto dropdown-animate" : "max-h-0"} overflow-y-auto lg:absolute mt-1 right-0 flex flex-col text-zinc-700 text-left`}>
                                                    ...
                                                    <span className="bg-white w-full px-3 py-1 lg:py-0.5 rounded-b hover:text-zinc-900 cursor-pointer">Área administrativa</span>
                                                </div>
                                            </Outclick>
                                        </div>
                                        :
                                        <div className="flex gap-4 text-sm">
                                            <button onClick={() => setTab("SignIn")} className="shrink-0 bg-gradient-to-b from-sky-600 to-cyan-400 lg:from-white lg:to-white hover:text-black text-gray-700 rounded py-1 px-4">Entrar</button>
                                            <button onClick={() => setTab("SignUp")} className="shrink-0 bg-gradient-to-b from-sky-600 to-cyan-400 lg:from-white lg:to-white hover:text-black text-gray-700 rounded py-1 px-4">Registrar-se</button>
                                        </div>
