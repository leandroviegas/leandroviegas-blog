...
const Comment = (comment: CommentProps) => {
  ...
  const HandleDeleteComment = () => {
    ...
    api
      .delete("/posts/comments", { headers, params: { _id: comment._id } })
      .then(() => {
      ...
      <article className="px-6 py-4 mt-3 text-base">
        <footer className="flex justify-between items-center">
          <div className="flex items-center">
            <Link
              to={`/blog/user/${comment?.user?.link}`}
              className="inline-flex items-center gap-2 mr-3 text-gray-900 dark:text-zinc-50 font-semibold"
            >
            ...
            <p className="text-xs text-gray-600 dark:text-zinc-400">
              {moment(comment.postedAt).format("LLL")}
            </p>
          </div>
            ...
            <Outclick onOutClick={() => setOptionsDrodown(false)}>
              {(user?._id === comment?.user?._id ||
                ...
                <button
                  ...
                  className="py-2 px-4 hover:bg-zinc-100 text-zinc-800 hover:text-zinc-900  dark:hover:bg-zinc-700 dark:text-zinc-100 dark:hover:text-zinc-50 rounded flex items-center gap-1"
                >
                  <BsThreeDots />
                </button>
              )}
                  ...
                  <ul className="py-1 text-sm text-gray-700 dark:text-gray-200">
                    <li className="hover:bg-gray-100 dark:hover:bg-zinc-600">
                      <button
                        ...
                        className="block gap-2 px-3 py-1 text-left w-full"
                      >
                        Apagar
                      </button>
                    </li>
        ...
        {comment.parentComment && (
          <span className="text-xs dark:text-zinc-400">
            Mencionou:{" "}
            <span>
              {comment.parentComment.content?.substring(0, 9)}
              {comment.parentComment.content.length > 10 && "..."}
            </span>{" "}
            de&nbsp;
            <Link
              className="text-cyan-500 hover:text-cyan-600 hover:underline"
              to={`/blog/user/${comment.parentComment?.user?.link}`}
            >
        ...
        {formSelected === "edit" ? (
          ...
          >
            <button
              ...
              type="button"
              className="text-gray-500 hover:text-gray-600 dark:text-gray-300 dark:hover:text-gray-400 py-2 rounded-lg text-sm font-medium flex items-center"
            >
              <VscClose />
              Cancelar
            </button>
        ...
        {formSelected === "" && user?._id && (
          <div className="flex items-center mt-2 space-x-4">
            <button
              onClick={() => setFormSelected("reply")}
              type="button"
              className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-300 hover:underline font-medium"
            >
              <BsChatRightText />
              Responder
            </button>
...
export { Section, CreateForm as Create, EditForm as Edit };
