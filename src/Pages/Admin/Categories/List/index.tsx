...
const Index = () => {
    ...
    const HandleSaveCategory = (evt: React.FormEvent<HTMLFormElement>) => {
        ...
        if (alerts.length === 0) {
            ...
            if (categoryForm?._id) {
                api.put("/categories", categoryForm, { headers: { authorization } }).then(resp => {
                ...
                api.post("/categories", categoryForm, { headers: { authorization } }).then(resp => {
                    setCategoryStatus("success");
                    HandleLoadCategories();
                    setPopup("");
                }).catch(err => {
    ...
    const HandleDeleteCategory = () => {
        ...
        api.delete("/categories", { headers: { authorization }, params: { _id: categoryForm?._id } }).then(resp => {
            setCategoryStatus("success");
            HandleLoadCategories();
            setPopup("");
        }).catch(err => {
                    ...
                    <Outclick callback={() => setPopup("")}>
                        {popup === "category-form" &&
                            <div data-aos="fade-down" className="bg-white rounded-lg w-96 max-w-screen">
                                <h1 className="text-xl font-semibold text-sky-800 bg-sky-100 rounded-t-lg px-4 p-2">{categoryForm?._id ? `Editar categoria - ${categoryForm.name}` : "Nova categoria"}</h1>
                                ...
                                <form onSubmit={HandleSaveCategory} className="m-4">
                                    ...
                                    <hr className="my-4 text-sky-800" />
                                    <div>
                                        <button type="submit" className="bg-sky-700 hover:bg-sky-800 text-white px-3 py-1 font-semibold">{categoryForm?._id ? `Salvar categoria` : "Criar categoria"}</button>
                                    </div>
                                </form>
                            </div>
                        }
                    ...
                    <div className="mt-8 mx-2 md:mx-6">
                        ...
                        <div className="flex gap-6 text-sky-800 mt-6">
                            ...
                            <div>
                                ...
                                <p className="text-sky-700 text-xl my-1 font-semibold">Que tal criar uma categoria nova?</p>
                                <button onClick={() => setPopup("category-form")} className="bg-sky-600 hover:bg-sky-700 transition font-semibold text-white px-3 py-1 rounded my-4">Nova Categoria</button>
                            </div>
                        </div>
                    </div>
                </div>
